...
namespace QRCodeAddIn
{
    ...
    public partial class AddIn : AddInBase
    {
        ...
        public override void OnCmd(ref EdmCmd poCmd, ref EdmCmdData[] ppoData)
        {
            ...
            var logFolder = this.Vault.TryGetFolderFromPath(logFolderPath);

            var logFileName = $"Log {Guid.NewGuid().ToString()}.txt";

            Logger.OutputLocation = logFolderPath;
            ...
            base.ForEachFile(ref ppoData, (IEdmFile5 file) => {
                ...
                Logger.LogToOutput(logFileName, $"{file.Name} - start processing...");
                ...
                try
                {
                    ...
                    var Str = string.Join("&",variableNames.Select(x=> $"{x}={values[Array.IndexOf(variableNames,x)]}"));
                    ...
                    if (pngFile != null)
                    {
                        ...
                        Logger.LogToOutput(logFileName, $"Updated png.");
                    ...
                    { 
                        ...
                        if (folder != null)
                        {
                            ...
                            if (pngFile != null)
                            {
                                ...
                                Logger.LogToOutput(logFileName, $"Added png and checked into the vault.");
            ...
            if (logFolder != null)
            {
                var id = logFolder.AddFile(poCmd.mlParentWnd, System.IO.Path.Combine(logFolder.LocalPath, logFileName));

                var logFile = this.Vault.GetObject(EdmObjectType.EdmObject_File, id) as IEdmFile5;

                logFile.UnlockFile(poCmd.mlParentWnd, "Added by QRCode add-in");
